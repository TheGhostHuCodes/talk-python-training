...
def create_account():
    ...
    success_msg(
        f"Created new account with id {state.active_account.id}"  # pylint: disable=no-member
    )
...
def register_cage():
    ...
    success_msg(f"Registered new cage with id {cage.id}")  # pylint: disable=no-member
...
def view_bookings():
    ...
    bookings = [(c, b) for c in cages for b in c.bookings if b.booked_date is not None]
